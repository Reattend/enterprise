...
import { Clock3, AlertTriangle, KeyRound } from 'lucide-react'
...
interface BillingStatus {
  ...
  byok?: { provider: string; status: string } | null
}
...
export function TrialBanner() {
  ...
  useEffect(() => {
    ...
    if (!enterpriseOrgsLoaded) { setData(null); return }
    ;(async () => {
      try {
  ...
  // A trial that has run out is the best moment to offer the free path:
  // their own key, forever, at no cost from us. Nothing surfaced this
  // before - the plan just quietly reverted and the user was left guessing.
  const lapsed =
    !!data.trialEndsAt &&
    ...
    !data.byok
  if (lapsed) {
    const keyHref = data.hasOrg && activeEnterpriseOrgId
      ? `/app/admin/${activeEnterpriseOrgId}/settings#ai-provider`
      : '/app/settings?tab=ai-provider'
    return (
      <div className="flex items-center gap-3 border-b bg-amber-50 px-4 py-2 text-sm text-amber-950 dark:bg-amber-950/30 dark:border-amber-900/40 dark:text-amber-100">
        <KeyRound className="h-3.5 w-3.5 shrink-0" />
        <span className="flex-1">
          <strong>Your Managed trial has ended.</strong> Connect your own AI key and keep Reattend free, forever - unlimited questions, nothing billed by us.
        </span>
        {(!data.hasOrg || data.isAdmin) && (
          <Link href={keyHref} className="shrink-0 rounded-md bg-amber-900 px-3 py-1 text-xs font-semibold text-amber-50 hover:opacity-90 dark:bg-amber-100 dark:text-amber-950">
            Connect a key
          </Link>
        )}
        <Link href="/app/settings/billing" className="shrink-0 underline underline-offset-2 font-medium hover:opacity-80">
          Or subscribe
        </Link>
      </div>
    )
  ...
  if (!activeEnterpriseOrgId || data.tier !== 'professional') return null
