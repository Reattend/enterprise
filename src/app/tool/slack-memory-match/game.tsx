...
export function SlackMemoryMatch({ slackReady = false }: { slackReady?: boolean }) {
  ...
  return (
    <MarketingShell>
      ...
      <section className="relative z-10 pt-16 md:pt-24 pb-12 px-5 text-center">
        ...
        emphasis="Match"
        emphasisJoiner=" "
        lede="A lightweight Slack game that shows how differently your team remembers the same moment."
      />
          ...
          <div className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-3">
            {/* Slack's own branded button only when the Slack app is actually
                configured. With no client ID it sent people to Slack's error page
                behind an official-looking button (2026-09-12). */}
            {slackReady ? (
              <>
                <a
              ...
              <span className="inline-flex items-center gap-2 text-[14px] font-semibold text-gray-500 border border-gray-200 bg-white px-6 py-3.5 rounded-full">
                Coming to Slack soon
              </span>
            )}
          </div>
        </motion.div>
