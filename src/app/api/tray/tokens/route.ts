...
export async function POST(req: NextRequest) {
  try {
    ...
    // No tier gate here. The extension is free on every plan; whether a
    // token can actually do anything is decided per request by
    // requireExtensionAccess() in the tray routes (needs a connected AI key
    // or Managed), which is the single place that rule lives.
